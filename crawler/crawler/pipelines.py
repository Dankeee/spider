...
class MySQLStoreLinkedinPipeline(object):
    ...
    def _do_upinsert1(self, conn, item):
        ...
        conn.execute("""
                select 1 from linkedinfo where idprofileitem = %s
        """, (linkmd5id,))
        ret = conn.fetchone()
        if ret:
            print "test point 13"
            ...
            """, (item['profile_url'], item['profile_img'], item['profile_name'], item['profile_headline'], item['profile_location'], item['profile_industry'], item['profile_current'], item['profile_previous'], item['profile_education'], item['profile_homepage'], item['profile_summary_bkgd'], item['profile_experience_bkgd'], item['profile_honors_bkgd'], item['profile_projects_bkgd'], item['profile_top_skills_bkgd'], item['profile_also_knows_bkgd'], item['profile_education_bkgd'], item['profile_organizations_bkgd'], item['profile_organizations_supports'], item['profile_causes_cares'], now, linkmd5id))
            print "test point 14"
        else:
            print "test point 15"
            conn.execute("""
            ...
            """, (linkmd5id, item['profile_url'], item['profile_img'], item['profile_name'], item['profile_headline'], item['profile_location'], item['profile_industry'], item['profile_current'], item['profile_previous'], item['profile_education'], item['profile_homepage'], item['profile_summary_bkgd'], item['profile_experience_bkgd'], item['profile_honors_bkgd'], item['profile_projects_bkgd'], item['profile_top_skills_bkgd'], item['profile_also_knows_bkgd'], item['profile_education_bkgd'], item['profile_organizations_bkgd'], item['profile_organizations_supports'], item['profile_causes_cares'], now))
            print "test point 16"
    ...
    def _do_upinsert2(self, conn, item):
        ...
        if ret:
            print "test point 17"
            conn.execute("""
                update companyitem set company_url = %s, company_name = %s, company_logo = %s, company_img = %s, company_description = %s, company_specialties = %s, company_website = %s, company_industy = %s, company_type = %s, company_headquarters = %s, company_size = %s, company_founded = %s, update = %s where idcompanyitem = %s
    ...
    def _get_linkmd5id1(self, item):
        #url进行md5处理，避免重复采集
        return md5(item['profile_url']).hexdigest()
